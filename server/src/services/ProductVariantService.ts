...
export const getProductVariantByIdService = (
  id: number,
...
export const getProductVariantsByProductIdService = (
  productId: number,
): ProductVariantInterface[] => {
  ...
  const variants =
    ProductVariantRepository.getProductVariantsByProductId(productId);
...
export const updateProductVariantService = (
  id: number,
  variant: Partial<ProductVariantInterface>,
): ProductVariantInterface | null => {
  if (!id || id <= 0) {
    throw new Error("Invalid product variant ID");
  }
