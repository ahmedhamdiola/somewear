...
export const createFeedback = (
  ...
  >(
    `
         ...
         (name,userId,productId,comment,rating) VALUES (?,?,?,?,?)
        `,
  );
...
export const getAllFeedbacks = (): FeedbackInterface[] => {
  const feedback = db.prepare<[], FeedbackInterface>(`SELECT * FROM feedback `);
  const result = feedback.all();
  return result;
};

//get feedback by productId
export const getFeedbackByProduct = (
  productId: number,
): FeedbackInterface[] => {
  const feedback = db.prepare<[number], FeedbackInterface>(
    `SELECT name,comment,rating FROM feedback WHERE productId = ?`,
  );
