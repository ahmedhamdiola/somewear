...
describe("ProductVariantService", () => {
  ...
  test("createProductVariantService — should throw on invalid productId", () => {
    expect(() =>
      ProductVariantService.createProductVariantService({ productId: 0, size: "L", stock: 5 })
    ).toThrow("Product ID is required");
  });

  test("createProductVariantService — should throw on negative stock", () => {
    expect(() =>
      ProductVariantService.createProductVariantService({ productId, size: "L", stock: -1 })
    ).toThrow("Stock must be a non-negative number");
  });

  test("createProductVariantService — should throw on empty size", () => {
    expect(() =>
      ProductVariantService.createProductVariantService({ productId, size: "", stock: 5 })
    ).toThrow("Size cannot be empty");
  });

  test("createProductVariantService — should throw when product does not exist", () => {
    expect(() =>
      ProductVariantService.createProductVariantService({ productId: 999999, size: "XL", stock: 5 })
    ).toThrow("Product not found");
  });

  // GET BY ID
  test("getProductVariantByIdService — should return variant by id", () => {
    const variant = ProductVariantService.getProductVariantByIdService(variantId);
    expect(variant).toBeDefined();
  ...
  test("getProductVariantsByProductIdService — should return variants array", () => {
    const variants = ProductVariantService.getProductVariantsByProductIdService(productId);
  ...
  test("updateProductVariantService — should update stock", () => {
    const updated = ProductVariantService.updateProductVariantService(variantId, { stock: 25 });
  ...
  test("updateProductVariantService — should throw on negative stock", () => {
    expect(() =>
      ProductVariantService.updateProductVariantService(variantId, { stock: -5 })
  ...
  test("deleteProductVariantService — should throw on invalid id", () => {
    expect(() =>
      ProductVariantService.deleteProductVariantService(-1)
    ).toThrow("Invalid product variant ID");
  });
